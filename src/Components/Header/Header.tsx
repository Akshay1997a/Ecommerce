...
import { Text, View, TextInput, FlatList, StatusBar } from "react-native";
...
export class Header extends Component<Props, State> {
  ...
  _renderCategories = ({ item, index }: any) => {
    ...
    return (
      <View>
        <TouchableNativeFeedback
          onPress={() => this.setState({ seletedCategorieId: item.id })}
        >
      ...
      <View style={style.container}>
        ...
        <View style={style.col2}>
          <View style={style.searchBar}>
            <SearchIcon />
            <TextInput style={style.inputText} placeholder="Search" />
            <TouchableOpacity>
              <ClearIcon />
            </TouchableOpacity>
          </View>
        ...
        <View style={style.col3}>
          <FlatList
            ...
            renderItem={({ item, index }) =>
              this._renderCategories({ item, index })
            }
          />
        </View>
      </View>
